// Shared receipt-analysis prompts for Anthropic and DeepSeek backends

export function buildScanReceiptPrompt(correctionsHint = "") {
  return `Scan this Polish receipt. Respond with ONLY raw JSON — no markdown, no backticks, no commentary.
...
export function buildParseTextReceiptPrompt(text, correctionsHint = "") {
  const today = new Date().toISOString().slice(0, 10);
  return `Parse the following Polish text into structured JSON. The text may be either a full receipt (paragon fiskalny), a simple shopping list, or MULTIPLE orders/receipts pasted together. Respond with ONLY raw JSON — no markdown, no backticks, no commentary.
...
- For SIMPLE SHOPPING LISTS (freeform text like "mleko 2zł"):
  ...
  * If no date is found, use today: "${today}".
...
export function buildParseJsonReceiptPrompt(jsonContent, source = null, correctionsHint = "") {
  ...
  return `You are given a JSON file containing receipt/purchase data. Map it to the structured schema below. The JSON may be from any store, loyalty app, or e-receipt system — detect the format automatically and extract all relevant data.${sourceHint}
